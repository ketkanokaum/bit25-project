...
async function fetchAllForecast() {
  const [forecastRows] = await pool.query(
    `SELECT f.province, f.year, f.month,
            ...
            c.baseline_mean,
            n.humidity_mean, n.precipitation_days AS normal_rain_days,
            n.temp_min, n.temp_max
     ...
     LEFT JOIN province_climate_normals n
       ON TRIM(f.province) = TRIM(n.province) AND f.month = n.month
